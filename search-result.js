...
async function loadResults(append = false) {
 if (loading) return;
  ...
  tv_movie: 10770
    ...
    url = `https://api.themoviedb.org/3/discover/movie?api_key=${API_KEY}&with_genres=${GENRE_MAP[genre.toLowerCase()]}&page=${page}`;
  }
  if (lang) {
  ...
  title.textContent = `${langNames[lang]} Movies`;
