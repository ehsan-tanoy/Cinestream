...
const HERO_IMG_URL = "https://image.tmdb.org/t/p/w780";
  ...
  loopData.forEach(movie => {
    ...
    card.innerHTML = `
      <img src="${movie.poster_path 
        ? HERO_IMG_URL + movie.backdrop_path 
        : 'blank_poster.png'}" 
...
<div class="bookmark-btn">
  <img class="bookmark-icon" src="icons/bookmark-new-outline.svg" />
</div>
  ...
  // check existing
  if (isInWatchlist(movie.id)) {
    bookmarkIcon.src = "icons/bookmark-new-filled.svg";
  ...
  if (already) {
    bookmarkIcon.src = "icons/bookmark-new-outline.svg";  
...
/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   ▶ CONTINUE WATCHING
   Vidking progress events → localStorage
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */

const CW_KEY = "cinestream_continue";

// Vidking থেকে progress event receive করো
window.addEventListener("message", (event) => {
  try {
    const msg = typeof event.data === "string" ? JSON.parse(event.data) : event.data;
    if (msg?.type !== "PLAYER_EVENT") return;

    const { event: evtName, id, mediaType, currentTime, duration, progress } = msg.data;

    // শুধু meaningful progress save করো (5% - 90%)
    if (!id || !duration || progress < 3 || progress > 90) return;
...
function saveContinueWatching(item) {
  const data = getContinueWatchingAll();
  // Same id হলে update করো
...
async function loadContinueWatching() {
  ...
  for (const item of data) {
    try {
      // TMDB থেকে poster + title আনো
      const res  = await fetch(
        `https://api.themoviedb.org/3/${item.mediaType === "tv" ? "tv" : "movie"}/${item.id}?api_key=${API_KEY}`
      );
      ...
      card.innerHTML = `
        <div style="position:relative;">
          ...
          <div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;
            ...
            <span class="material-symbols-rounded" style="font-size:48px;color:#fff;opacity:0.9;">
              play_circle
            </span>
          </div>
        </div>
      ...
      // Click → player.html with progress resume
      card.onclick = () => {
        const t = Math.floor(item.currentTime);
        const url = item.mediaType === "tv"
          ? `stream.html?type=tv&id=${item.id}&s=${item.season||1}&e=${item.episode||1}&progress=${t}`
          : `stream.html?type=movie&id=${item.id}&progress=${t}`;
