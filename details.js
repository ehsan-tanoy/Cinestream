...
function renderFileInfo(dfData, fmData) {
  ...
  box.innerHTML = `
    ${fileName ? `
      <div style="font-size:11px;color:rgba(255,255,255,0.4);font-family:Inter;
...
function addFTPButtons(dfData, fmData) {
  ...
  if (dfData?.direct_link) {
    ...
    btn.innerHTML = `<span class="material-symbols-rounded" style="font-size:14px;">dns</span> FTP 1${label ? ' · ' + label : ''}`;
