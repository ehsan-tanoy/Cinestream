      ...
      <div class="cast-section">
        ...
        <div class="cast-row">
          ${cast.slice(0, 10).map(actor => `
            <div class="cast-card">
              <img src="${actor.profile_path ? IMG_URL + actor.profile_path : ''}" />
              <p>${actor.name}</p>
            </div>
          `).join("")}
        </div>
...
function updateWatchBtn(id) {
  const watchIcon = watchBtn.querySelector("span");

  if (isInWatchlist(id)) {
    watchIcon.textContent = "bookmark_added";
  } else {
    watchIcon.textContent = "bookmark";
  }
}
  updateWatchBtn(movie.id);
