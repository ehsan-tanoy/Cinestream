  ...
  Watch Now
      ...
      <button class="download-btn" id="watchlistBtn" style="font-family:Inter;font-weight:bold;font-size:16px">
        ...
        <div style="margin-top: 28px;">
            <h3 style="margin:0;padding:0;">Story</h3>
            <p style="margin:15px 0 0 0;padding:0;">${movie.overview}</p>
        </div>
      ${
        trailer
          ? `
...
function renderFileInfo(dfData, fmData) {
  ...
  const lang     = dfData ? getLanguageLabel(dfData) : (fmData?.language || null);
  const quality  = primary.quality   || null;
  const size     = dfData?.file_size || null;
  const fileName = primary.file_name || null;

  if (!lang && !quality && !size) return;
  ...
  box.innerHTML = `
      ...
      <div style="font-size:13px;color:rgba(255,255,255,1);font-family:Inter;
                  margin-bottom:10px;line-height:1.5;">
        For smoother experience, Stream it on your mobile player!
      </div>
    
    
    <div id="ftpBtns" style="margin-top:12px;display:flex;flex-wrap:wrap;gap:8px;"></div>
  `;
...
function addFTPButtons(dfData, fmData) {
  const container = document.getElementById("ftpBtns");
  if (!container) return;
  ...
  if (dfData?.direct_link) {
    const q    = dfData.quality  || '';
    const s    = dfData.file_size|| '';
    const label= [q, s].filter(Boolean).join(' / ');
    const btn  = document.createElement('button');
    btn.style.cssText = btnStyle;
    btn.innerHTML = `<span class="material-symbols-rounded" style="font-size:14px;">dns</span> FTP 1${label ? ' · ' + label + 'GB': ''}`;
    btn.onclick   = () => openWithIntent(dfData.direct_link);
    container.appendChild(btn);
  }

  if (fmData?.direct_link) {
    const q    = fmData.quality  || '';
    const s    = fmData.file_size|| '';
    const label= [q, s].filter(Boolean).join(' / ');
    const btn  = document.createElement('button');
    btn.style.cssText = btnStyle;
    btn.innerHTML = `<span class="material-symbols-rounded" style="font-size:14px;">dns</span> FTP 2${label ? ' · ' + label : ''}`;
    btn.onclick   = () => openWithIntent(fmData.direct_link);
    container.appendChild(btn);
  }
}

function openVidking(tmdbId) {
  window.open(`stream.html?type=movie&id=${tmdbId}`, '_blank');
}
...
async function setupPlayButtons(title, year, tmdbId) {
  ...
  // Watch Now → Vidking (always available via TMDB ID)
  ...
  playBtn.innerHTML = `
    ...
    Watch Now
  `;
  playBtn.onclick = () => openVidking(tmdbId);

  // FTP buttons
  addFTPButtons(dfData, fmData);
}
