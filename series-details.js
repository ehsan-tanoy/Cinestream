...
async function fetchSupabaseEpisodes(seriesTitle) {
  ...
  try {
    ...
    // Step 1 — Exact title match আগে try করো (fast)
    let url = `${SUPA_URL}/rest/v1/series`
      + `?select=season,episode,direct_link`
    ...
    // Step 2 — Exact না পেলে fuzzy search
    if (!data?.length) {
